...
Board membership and underlying resource permissions jointly determine visible work.

```mermaid
flowchart TD
  accTitle: Several kinds of Board work
  accDescr: Board membership and underlying resource permissions jointly determine visible work.
  A[Board membership] --> B[Visible work items]
  B --> C[Proposal]
  B --> D[Commission task]
  B --> E[External issue]
...
See [Board Data and Route Reference](/documentation/orbiters.reference.board-data-and-routes) for the detailed contract.
