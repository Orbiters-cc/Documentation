...
Gallery variants keep the same mounted image. The live image uses the nearest legal crop immediately, stretched to the pointer dimensions. At each variant boundary, a temporary image layer crossfades the outgoing crop over 360 milliseconds. The actual gallery image and signed-source state never remount. Other widget variants keep the outgoing layer opaque beneath the incoming 360-millisecond fade, then remove it; this prevents a dark intermediate frame. Documentation supports 3×1, 2×1, 1×1, 1×2 and 2×2 in both the registry and server preference validation.
...
`GalleryWidget` captures its source card's bounds and opens `CardPreviewDialog`. Framer Motion animates the actual width, height and position between the source and centered window, without scaling image or text content. The surface is opaque throughout; a separately animated backdrop, inset close control and focus restoration remain. Content is measured at its target width, and scrollbars stay hidden until the bounds settle. Age verification retains its existing shared-layout implementation.

Existing `GalleryImage` keeps signed-source refresh and access handling. Its already-loaded thumbnail and natural dimensions are passed into the preview. The thumbnail remains visible during expansion and while the full image loads; `ProgressiveGalleryPicture` crossfades to the decoded full image over 350 milliseconds. A failed full-size read leaves the thumbnail visible. `GalleryImageDetails` is shared with the gallery page: author, posted date, reactions, attachment position and reporting controls remain consistent. Content scrolls when details or the report form exceed the viewport. Reduced motion removes spatial expansion.
...
Unpinned gallery widgets initially choose a registered size from the image width/height and the rendered column geometry. The closest aspect ratio wins, with smaller tiles preferred for near ties. Missing dimensions use 1×1. `packFeedTail` searches order and registered size alternatives in bounded batches of six throughout the new feed, retaining at most eight candidates per step, to minimize occupied rows and unused cells, allowing smaller images before gaps accumulate. It penalizes changes to preferred proportions and preserves chronological order among individual events. Cached placements reuse their chosen sizes on pagination: already-visible widgets and pins never join this search. Resizing the browser recomputes feed sizes; pinned dimensions remain the user's choice.

## Desktop content previews

At viewport widths of 1024 pixels or more, creator, asset, latest/archive article and documentation links expand their card using `CardPreviewDialog`, including its opaque surface, direct bounds animation and scrollbar handling. `WidgetPreview` uses the same Framer Motion runtime as HeroUI, a fading backdrop, inset close button, focus restoration and reduced-motion handling. Explicit modified clicks retain normal link behavior. Below the breakpoint, links navigate directly; narrowing an open preview closes it.
