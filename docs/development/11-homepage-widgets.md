...
Unpinned gallery widgets initially choose a registered size from the image width/height and the rendered column geometry. The closest aspect ratio wins, with smaller tiles preferred for near ties. Missing dimensions use 1×1. `packFeedTail` searches order and registered size alternatives for the final six new widgets, retaining at most eight candidates per step, to reduce unused cells. It penalizes changes to preferred proportions and preserves chronological order among individual events. Cached placements reuse their chosen sizes on pagination: already-visible widgets and pins never join this search. Resizing the browser recomputes feed sizes; pinned dimensions remain the user's choice.
